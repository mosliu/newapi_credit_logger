...
def test_scheduler_reload_jobs_sets_misfire_grace(monkeypatch) -> None:
    ...
    assert added_job_kwargs["next_run_time"] is not None


def test_scheduler_reload_jobs_keeps_existing_job_schedule(monkeypatch) -> None:
    monkeypatch.setattr(
        scheduler_service,
        "get_settings",
        lambda: SimpleNamespace(
    ...
    monkeypatch.setattr(service.scheduler, "remove_job", lambda _job_id: None)
